	...
	case VK_UP:
		{
			...
			//  we jump there
			if(SelectedLine>0){	// we a an even smaller line there
				...
				if(olds > 0){ 
					...
					//Shift 1 line
					lnPageStart--;
					if(lnPageStart<0){ lnPageStart = 0; break; }
					CClientDC dc(this);
					CRect rc;
					GetClientRect(&rc);
					dc.BitBlt( 0,SideMargin + CharHeight*2,rc.right, CharHeight * (numLines - 2),
								(CDC*)&dc,0, SideMargin + CharHeight, SRCCOPY);
	...
	case VK_DOWN:
		{
			...
			// if the new selected line is not shown then the
			//  we jump there
			if(SelectedLine<totNumLines-1){	// we a an even greater line there
				int old  = SelectedLine;
				int olds = old - lnPageStart;
				SelectedLine++;
				if(olds >= -1 && olds < numLines - 1){ 
					...
					//Shift 1 line
					lnPageStart++;
					if(lnPageStart>totNumLines - numLines){ lnPageStart = totNumLines-numLines; break; }
					CClientDC dc(this);
					CRect rc;
					GetClientRect(&rc);
					dc.BitBlt( 0,SideMargin ,rc.right, CharHeight * (numLines - 2),
								(CDC*)&dc,0, SideMargin+CharHeight, SRCCOPY);
					drawLine(SelectedLine);
					drawLine(old);
				}
...
void CBaseWnd::OnVScroll(UINT nSBCode, UINT nPos, CScrollBar * pScrollBar){
	...
	case SB_LINEDOWN:
		if(lnPageStart+numLines<totNumLines){
				CRect rc;
				GetClientRect(&rc);
				dc.BitBlt( 0,SideMargin,rc.right, CharHeight * (numLines - 1),
								(CDC*)&dc,0, SideMargin + CharHeight, SRCCOPY);
				lnPageStart++;
	...
	case SB_LINEUP:
		if(lnPageStart){
				CRect rc;
				GetClientRect(&rc);
				dc.BitBlt( 0,SideMargin + CharHeight,rc.right, CharHeight * (numLines - 1),
								(CDC*)&dc,0, SideMargin, SRCCOPY);
				lnPageStart--;
				drawLine(lnPageStart);
		}
		break;
