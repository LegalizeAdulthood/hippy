...
void CMemDumpWnd::OnLButtonDown(UINT nFlags, CPoint point){
	...
	if(SelectedByte > 7) SelectedByte = -1;
...
void CMemDumpWnd::OnKeyDown(UINT nChar, UINT nRepCnt, UINT nFlags){
	...
	switch(nChar){
	case VK_LEFT:
		if(SelectedByte>0) SelectedByte--;
		drawLine(SelectedLine);
		editMask = 0x10;
		break;
	case VK_RIGHT:
		if(SelectedByte<7) SelectedByte++;
		drawLine(SelectedLine);
		editMask = 0x10;
		break;
	}
...
void CMemDumpWnd::drawLine(LINENUMBER lnActualNum){
	...
	for(i=0;i<8;i++) b[i] = pbMemoryBase->Read(addr+i, true);
