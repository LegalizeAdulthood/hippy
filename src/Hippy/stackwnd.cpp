...
void CStackWnd::ScrollDown(int nl){
	...
	CRect rc;
	GetLineRect(0, &rc);
	dc.BitBlt( 0,SideMargin + CharHeight * nl,rc.right, CharHeight * (totNumLines - nl),
...
void CStackWnd::ScrollUp(int nl){
	...
	CRect rc;
	GetLineRect(0, &rc);
	dc.BitBlt( 0,SideMargin, rc.right, CharHeight * (totNumLines - nl),
							(CDC*)&dc,0, SideMargin+ CharHeight * nl, SRCCOPY);
}
...
void CStackWnd::Pop(BYTE code, bool paint){
	...
	case 0x33: /*PULB*/
		ScrollUp(1);
		if(paint) ClearLine(totNumLines-1);
		PopEx(1);
		break;
	case 0x39: /*RTS*/
		ScrollUp(2);
