...
void CAsmEditorWnd::OnSize(UINT nType, int cx, int cy){
	...
	if(!IsWindow(Editor.m_hWnd)) return;
...
int CAsmEditorWnd::CompileCode(){
	...
	if(!CreatePipe(&rd, &wr, &sa, 0)){
		...
		CreateProcess( NULL, buffer, NULL, NULL, true, CREATE_NEW_CONSOLE /*creation flags*/,
...
CAsmEditorWnd::CAsmEditorWnd(CMDIFrameWnd * pParent, LPCSTR lpcFileName){
	...
	buildWnd.Create(WS_CHILD | WS_VISIBLE | ES_MULTILINE| 
		WS_VSCROLL|WS_TABSTOP|WS_BORDER , 
		rectDefault, this, 102);
	buildWndHeight = 100;
