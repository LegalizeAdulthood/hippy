...
char CDeviceFile::GetNextToken(){
	...
	while(xmlTag.szData.GetLength())
		switch(state){
			case 0:
				{
					if(xmlTag.szData[0] == 'P') state = 1;
					else
						if(xmlTag.szData[0] != ' '){
							ret = xmlTag.szData[0];
							switch(ret){
							case '~': ret = (char)bitNOT; break;
							case '(': ret = (char)leftPrnt; break;
							case '.': ret = (char)bitAND; break;
							case '^': ret = (char)bitXOR; break;
							case '|': ret = (char)bitOR; break;
							}
...
char CDeviceFile::GetString(CString & szString){
	...
	while(c==' ' || c== '\t'){ c = fgetc(file); if(feof(file)) return 0; }
	while(c!='\n' && c!=' ' && c!= '\t' && c!='>' && c!='='){
		szString += c;
		c = fgetc(file);
	}
...
label1:
	...
	if(str=="HDF"){
		...
		GetField("id", str);
		xmlTag.id = atoi(str);
		if(!GetUntilChar('<', xmlTag.szData)) return false;
		GetUntilChar('>', str);
		if(str!="/AI") return false;
		CompileData();
	}
		...
		if(!GetUntilChar('<', xmlTag.szData)) return false;
		GetUntilChar('>', str);
		if(str!="/CHIP_SELECT") return false;
		CompileData();
	}
	else if(str=="/DEVICE"){
		xmlTag.type = ttDevice;
...
int CDeviceFile::ParseFile(CWnd * parent, CString szFileName, 
						   CDeviceArray & devArr, 
						   AddrResEntry * AddrResTbl){
	CString			szName;
	CStringArray	csEqn;
